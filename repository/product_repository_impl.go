...
func (repository ProductRepositoryImpl) Save(ctx context.Context, tx *sql.Tx, product domain.Product) domain.Product {
	...
	result, err := tx.ExecContext(ctx, sql, product.Name, product.Price, product.CreatedAt, product.UpdatedAt)
...
func (repository ProductRepositoryImpl) FindById(ctx context.Context, tx *sql.Tx, productId int) (domain.Product, error) {
	...
	if rows.Next() {
		err := rows.Scan(&product.Id, &product.Name, &product.Price, &createdAtStr, &updatedAtStr)
		helper.PanicIfError(err)

		product.CreatedAt = helper.StrToDateTime(createdAtStr)
		product.UpdatedAt = helper.StrToDateTime(updatedAtStr)
	} else {
